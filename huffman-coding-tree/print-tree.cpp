...
int main() {
    ...
    for (auto e : encoded){
        ...
        // Store ext. ascii chars to be printed later
        else {                
            if (int(e.first) == -108)
                extendedAscii.push_back(make_pair("{closed quotation}",e.second));
            else if (int(e.first) == -109)
                extendedAscii.push_back(make_pair("{open quotation}",e.second));
            else if (int(e.first) == -110)
                extendedAscii.push_back(make_pair("{apostrophe}",e.second));
            else if (int(e.first) == -105)
                extendedAscii.push_back(make_pair("{ext. hyphen}",e.second));
            else
                extendedAscii.push_back(make_pair(to_string(e.first),e.second));       // Unknown ascii char
        }
    }

    // Print extended ascii information 
    for (auto e : extendedAscii){
        // Unknown ext. ascii (prefixes for known ext. ascii)
        if (e.first[0] == '-')
            cout << char(stoi(e.first)) << "  (" << e.first << ") : " << e.second << endl;
    ...
    // Convert text file into a string of huffman values
    ofstream outfile;
